...
run=True
def fall_detector(camera,detection_input_size=384,pose_input_size='224x160',pose_backbone='resnet50',show_detected=False,show_skeleton=True,save_out='',device='cpu'):
    ...
    fps_time = 0
    f = 0
    while run==True and cam.grabbed():
        f += 1
        frame = cam.getitem()
        image = frame.copy()
	...
	# Show Frame.
        ...
        #cv2.imshow('HumanFallDetector', frame)
...
if __name__ == "__main__":
    ...
    t1 = threading.Thread(target=fall_detector, args=[str(url)])
    t1.start()
    while True:
